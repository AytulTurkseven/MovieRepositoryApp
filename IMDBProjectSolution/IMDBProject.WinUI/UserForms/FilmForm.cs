...
namespace IMDBProject.WinUI.UserForms
{
    public partial class FilmForm : Form
    {
        ...
        private void FilmForm_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                lnkYonetmen.Text = YonecticiListe[0].DirectorName+" "+YonecticiListe[0].DirectorSurname;
                lnkYonetmen.Tag = YonecticiListe[0].DirectorID;
                lnkYonetmen.Visible = true;

                lnkYonetmen2.Text = YonecticiListe[1].DirectorName + " " + YonecticiListe[1].DirectorSurname; ;
            ...
            string year= Convert.ToString(_filmService.GetByIdFilmService(FilmFilterForm.filmFiltreDTONesne.FilmID).ReleaseDate.Year);
            string month= Convert.ToString(_filmService.GetByIdFilmService(FilmFilterForm.filmFiltreDTONesne.FilmID).ReleaseDate.Month);
            string day= Convert.ToString(_filmService.GetByIdFilmService(FilmFilterForm.filmFiltreDTONesne.FilmID).ReleaseDate.Day);
            lblVizyonTarihi.Text = day + "-" + month + "-" + year;
